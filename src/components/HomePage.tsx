...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      index += 1;
      setDisplayedFortune(fortune.slice(0, index));
        ...
        <div className="max-w-2xl mx-auto px-4 w-full">
          ...
          <div className="flex flex-col items-center gap-6">
            <div className="flex flex-col items-center gap-4">
              ...
              {user ? (
                <Button 
                  ...
                  disabled={isLoading}
                >
                  {isLoading ? "loading..." : "seek enlightenment"}
                </Button>
              ) : (
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button size="lg" className="px-8 py-3 hover:cursor-not-allowed">
                      get fortune
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
            ...
            <div className="w-full">
              <div className={`px-8 py-8 rounded-lg border shadow-sm bg-background transition-opacity duration-300 ${showCard ? 'opacity-100' : 'opacity-100'}`}>
                <div className="min-h-32 max-h-64 overflow-y-auto whitespace-pre-wrap text-center text-xl leading-relaxed">
                  {isLoading ? (
                    ...
                    <>
                      {displayedFortune}
                      {isRevealing && <span className="ml-0.5 inline-block w-3 h-6 align-baseline bg-primary/70 animate-pulse" />}
                    </>
                  ) : (
